...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-slate-200/60 shadow-sm">
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-slate-500 hover:text-red-600 hover:bg-red-50 transition-all duration-200"
              >
      ...
      {/* Mobile Navigation */}
      {isOpen && (
        <div className="md:hidden glass border-t border-slate-200/60 animate-slide-up max-h-[80vh] overflow-y-auto">
          <div className="px-4 py-3 space-y-1">
            ...
            {user ? (
              <button
                onClick={handleLogout}
                className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-semibold text-slate-500 hover:text-red-600 hover:bg-red-50 transition-all duration-200 mt-2 border border-slate-200"
              >
                <LogOut className="w-4 h-4" />
                Logout
              </button>
